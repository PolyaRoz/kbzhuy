...
▶ А. «Завтра/сегодня съем/поужинаю X (бургер, пицца, ресторан)»:
  Это ровно 4 (или 3) tool-вызова, ВСЕГДА в этом порядке. После 4-го — текстовый ответ. ВСЁ.
  ...
  2. register_deviation(description='X (вместо ужина)', kcal, protein_g, fat_g, carbs_g)
     ...
     Ответ tool вернёт {id: N} — запомни N.
  3. recalculate_plan(deviation_id=N из шага 2)  ← ОБЯЗАТЕЛЬНЫЙ ШАГ.
  4. Если у замененного приёма есть [контейнер X]: update_container(container_label='X', status='frozen', note='заменён на бургер')
  ...
  ❌ НЕ вызывай get_week_plan после register_deviation — meal_id уже в системном контексте.
  ❌ НЕ вызывай build_meal_plan — это удалит весь план!
